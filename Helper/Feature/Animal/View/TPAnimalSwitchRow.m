...
@interface TPAnimalSwitchCell ()
@property (nonatomic, strong) UILabel *titleLable;
@property (nonatomic, strong) UISwitch *switchBtn;
@end
@implementation TPAnimalSwitchCell
- (void)setupSubviews {
...
@implementation TPAnimalSwitchRow
@dynamic cell;
- (instancetype)init {
    self = [super init];
    if (self) {
        [self setCellClass:TPAnimalSwitchCell.class];
    }
    return self;
}
...
+ (instancetype)sterilizationRowWithId:(NSString *)rowId {
    TPAnimalSwitchRow *row = [TPAnimalSwitchRow rowWithID:rowId];
    row.title = @"是否绝育";
    return row;
}
/// 驱虫
...
- (void)tp_tableViewPreparedCell:(TPAnimalSwitchCell *)cell proxy:(TPTableViewProxy *)proxy indexPath:(NSIndexPath *)indexPath {
