...
@interface TPAnimalAlertCell ()
@property (nonatomic, strong) UILabel *titleLable;
@property (nonatomic, strong) UILabel *valueLabel;
@property (nonatomic, strong) UIImageView *arrowImage;
@end

@implementation TPAnimalAlertCell
- (void)setupSubviews {
...
@interface TPAnimalAlertRow ()

@end

@implementation TPAnimalAlertRow
@dynamic cell;
- (instancetype)init {
    self = [super init];
    if (self) {
        [self setCellClass:TPAnimalAlertCell.class];
    }
    return self;
}
...
+ (instancetype)categoryRowWithId:(NSString *)rowId {
    TPAnimalAlertRow *row = [TPAnimalAlertRow rowWithID:rowId];
    row.title = @"萌宠种类";
    return row;
}
...
- (void)tp_tableViewPreparedCell:(TPAnimalAlertCell *)cell proxy:(TPTableViewProxy *)proxy indexPath:(NSIndexPath *)indexPath {
