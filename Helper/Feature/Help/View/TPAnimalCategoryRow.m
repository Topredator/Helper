...
@interface TPAnimalCategoryCell : TPUIBaseTableViewCell
/// 年龄
...
@end
@implementation TPAnimalCategoryCell
- (void)setupSubviews {
...
@interface TPAnimalCategoryRow ()
@property (nonatomic, strong) TPAnimalModel *model;
@end

@implementation TPAnimalCategoryRow
- (instancetype)init {
    self = [super init];
    if (self) {
        [self setCellClass:TPAnimalCategoryCell.class];
    }
    return self;
}
+ (instancetype)rowWithModel:(TPAnimalModel *)model {
    TPAnimalCategoryRow *row = [TPAnimalCategoryRow row];
    row.model = model;
    return row;
}
- (void)tp_tableViewPreparedCell:(TPAnimalCategoryCell *)cell proxy:(TPTableViewProxy *)proxy indexPath:(NSIndexPath *)indexPath {
