...
@interface TPReleaseAdoptInputCell ()
@property (nonatomic, strong) UILabel *titleLable;
@property (nonatomic, strong) TPLimitTextField *textfield;
@end

@implementation TPReleaseAdoptInputCell
- (void)setupSubviews {
...
@implementation TPReleaseAdoptInputRow
@dynamic cell;
- (instancetype)init {
    self = [super init];
    if (self) {
        [self setCellClass:TPReleaseAdoptInputCell.class];
    }
    return self;
}
...
+ (instancetype)nameRowWithId:(NSString *)rowId {
    TPReleaseAdoptInputRow *row = [TPReleaseAdoptInputRow rowWithID:rowId];
...
+ (instancetype)numberRowWithId:(NSString *)rowId {
    TPReleaseAdoptInputRow *row = [TPReleaseAdoptInputRow rowWithID:rowId];
    row.title = @"萌宠编号";
    row.cellPrepared = ^(__kindof TPTableRow * _Nonnull rowData, TPReleaseAdoptInputCell * _Nonnull cell, TPTableViewProxy * _Nonnull proxy, NSIndexPath * _Nonnull indexPath) {
...
- (void)tp_tableViewPreparedCell:(TPReleaseAdoptInputCell *)cell proxy:(TPTableViewProxy *)proxy indexPath:(NSIndexPath *)indexPath {
