...
@interface TPAdoptPublisherCell : TPUIBaseTableViewCell
...
@implementation TPAdoptPublisherCell
- (void)setupSubviews {
...
@interface TPAdoptPublisherRow ()
@property (nonatomic, strong) TPUserModel *model;
@end

@implementation TPAdoptPublisherRow
- (instancetype)init {
    self = [super init];
    if (self) {
        [self setCellClass:TPAdoptPublisherCell.class];
    }
    return self;
}
+ (instancetype)rowWithModel:(TPUserModel *)model {
    TPAdoptPublisherRow *row = [TPAdoptPublisherRow row];
    row.model = model;
    return row;
}
- (void)tp_tableViewPreparedCell:(TPAdoptPublisherCell *)cell proxy:(TPTableViewProxy *)proxy indexPath:(NSIndexPath *)indexPath {
