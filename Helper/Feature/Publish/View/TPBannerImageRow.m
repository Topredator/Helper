...
@implementation TPBannerImageRow
@dynamic cell;
- (instancetype)init {
    self = [super init];
    if (self) {
        [self setCellClass:TPBannerImageCell.class];
    }
    return self;
}
+ (instancetype)row {
    TPBannerImageRow *row = [TPBannerImageRow rowWithID:kTPPublishSingleImageRowKey];
    return row;
}
...
- (void)tp_tableViewPreparedCell:(TPBannerImageCell *)cell proxy:(TPTableViewProxy *)proxy indexPath:(NSIndexPath *)indexPath {
    ...
    [[cell.addBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        pickerVC.total = 4;
        pickerVC.namePrefix = @"publish_announcement_";
